...
public class UsersController : BaseController
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(PaginatedResponse<ListUsersResponseItem>), StatusCodes.Status200OK)]
    ...
    {
        var command = new ListUsersCommand { Page = page, Size = size, Order = order };
        var result = await _mediator.Send(command, cancellationToken);
        var items = _mapper.Map<List<ListUsersResponseItem>>(result.Data);
        var pagedList = new PaginatedList<ListUsersResponseItem>(items, result.TotalItems, result.CurrentPage, size);
